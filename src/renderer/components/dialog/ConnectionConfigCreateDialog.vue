<!--
    ConnectionConfigCreateDialog.vue
    描述：创建连接配置弹窗。支持手动新建、从已有连接复制、分组选择和连接测试。
 -->
<template>
    <el-dialog v-model="dialogVisible" width="800px"
               ...
               @close="handleCancel">
        <template #header>
            <!-- 弹窗标题：连接配置创建入口使用连接图标，和左侧连接管理语义保持一致。 -->
            <DialogTitle :icon="LinkThree" :title="t('connectionDialog.createTitle')"/>
        </template>
        ...
        <template #footer>
            <div class="dialog-footer">
                ...
                <div class="footer-right">
                    ...
                    <el-button type="primary" @click="handleSubmit">{{ t('connectionDialog.actions.create') }}</el-button>
                </div>
            </div>
        </template>
    </el-dialog>
...
import {Clue, Info, LinkThree, Lock, More, Selected} from '@icon-park/vue-next'
...
// 对外事件：同步弹窗显示状态，并在关闭后通知父组件清理临时数据。
    ...
    if (newValue) {
        ...
        if (props.copyFromConnectionConfig) {
            ...
            if (props.defaultGroupName) {
                formData.group_name = normalizeConnectionGroupName(props.defaultGroupName)
            }
        }
        // 重置到基础信息标签页
...
const loadCopyConnectionData = () => {
    ...
    Object.assign(formData, {
        ...
        name: `${config.name || ''} ${t('connectionDialog.copySuffix')}`.trim(),
...
// 基础状态 store：创建成功后根据搜索模式决定刷新搜索结果还是全量连接列表。
...
const handleSubmit = async () => {
    try {
        ...
        ElMessage.success(t('connectionDialog.messages.createSuccess'))
        ...
        ElMessage.error(t('connectionDialog.messages.createFail') + (error.message || t('connectionDialog.messages.unknownError')))
    }
}
